...
static void userDriverSetParam(float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear){
	...
	else
	{
		...
		*/
		...
		if (c.r<=60)
		{
			expectedSpeed = constrain(45,130,c.r*c.r*(-0.046)+c.r*5.3-59.66);//45
		}
		/*else if (c.r == 500)
		{
		...
		{
			/*if (dist < 0.1 && abs(D_err) <0.1 && c.r==500)
			{
				expectedSpeed = constrain(100, 220-dist*200-abs(D_err)*200, c.r * 1.8);
			}
			else
			{
				expectedSpeed = constrain(100, 150-dist*10, c.r * 1.4);
			}*/
			expectedSpeed = constrain(100, 270-abs(D_err)*25 , c.r * 1.4); //290 20 link speed with D_err
		}
		...
		if (curSpeedErr > 0)
		{
			...
			if (abs(*cmdSteer)<0.6 || _speed<30) // help starting
			{
		...
		*/
		...
		//get the error 
		int obj = 8 + 0.7 * startPoint;// take current speed into account
		D_err = -atan2(_midline[obj][0],_midline[obj][1]);//only track the aiming point on the middle line

		//the differential and integral operation 
		D_errDiff = D_err - Tmp;
		D_errSum = 0.1*D_errSum + D_err;
		Tmp = D_err;

		//set the error and get the cmdSteer
		*cmdSteer =constrain(-1.0,1.0,kp_d * D_err + ki_d * D_errSum + kd_d * D_errDiff + 1.1 * _yaw);//1.5 associate steer with yaw.   PID of D_err
